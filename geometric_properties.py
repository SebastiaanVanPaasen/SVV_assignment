...
def boom_areas(aileron, stiffner, a_spar, n_discretize):  # pass aileron, stiffner as tupel, see below
    ...
    for i in range(n_discretize):
        y_pos_booms = np.append(y_pos_booms, map_s_to_yz(i*distance_boom, h_aileron, cord_aileron))
    boom_areas = np.array([])
    for i in range(n_discretize):
...
def main():
    ...
    boom_areas(aileron,stiffner, a_spar,n_discretize)
    return
